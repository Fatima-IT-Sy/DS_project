...
// Constructor to initialize the empty list of students
 ...
 // Inserts a student into the list in sorted order by studentId. If a student with the same ID already exists, insertion fails.
    public boolean add(IStudent student){
     ...
     students.findFirst();
     // Case 2: Insertion at the beginning (if the new ID is smaller than the first one)
    ...
    // We find the 'prevId' which is the ID of the last student smaller than the new one
     int prevId;
     while(!students.last() && students.retrieve().compareTo(student)==-1){
     prevId=students.retrieve().getStudentId();
      students.findNext();
     }
     //we have the largest id less than new Id, we will start from first of the list and stop at id=prevId
    ...
    // List to store matching students
     ...
     if(students.empty()) return nameList;
     students.findFirst();
   ...
   // List to store matching students
    ...
    // Case1: Return empty list if no students exist
    if(students.empty()) return majorList;
   ...
   //Returns all students in the specified year level.
    ...
    // Case1: Return empty list if no students exist
    if(students.empty()) return yearLevelList;
    students.findFirst();
    // Case2: Traverse all elements except the last
    while(!students.last()){
   ...
   // List to store matching students
    ...
    // Case1: Return empty list if no students exist
    if(students.empty()) return allStudents;
    students.findFirst();
    // Case2: Traverse all elements except the last to isert them all into a new list
    while(!students.last()){
